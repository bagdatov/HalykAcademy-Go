...
curl --user admin:admin -v --raw http://localhost:8080/app/user/0 
echo

echo check existing user without autothrization and -v --raw flag for trailers
curl -v --raw http://localhost:8080/app/user/0 
echo

echo "register new user"
curl --user admin:admin --data "username=test1&password=test1" http://localhost:8080/app/user/1
echo
echo
...
curl --user admin:admin http://localhost:8080/app/user/1
echo
echo
